...
p,'^http')or string.match(p,'^rbxassetid://')then o.MeshId=p o.InitialSize=b.
Vector3.new(init.x,init.y,init.z)end if string.match(q,'^http')or string.match(q
,'^rbxassetid://')then o.TextureID=q end end local p i,p=e.u32(j,i)assert(p<=
100000,`child count too high: {p} for {k} '{m}'`)for q=1,p do local r i,r=
unpack(j)if(n and o and r)then r.Parent=o end end return i,if n then o else nil
end c.serve(8008,{handleWebSocket=function(j)local k=buffer.tostring(f.decode(
buffer.fromstring(j:next())))print(`received payload ({#k} bytes)`)_,h=unpack(
buffer.fromstring(k))d.writeFile('DataModel.rbxl',b.serializePlace(h))print
'done'end})print(`serving on ws://localhost:8008`)